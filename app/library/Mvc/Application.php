...
class Application extends \Phalcon\Mvc\Application
{
    ...
    protected function _registerRouter()
    {
        if (production()) {
            ...
            file_put_contents(CACHE . 'routes.php', '<?php return ' . var_export($routes, true) . ';');
        }
    ...
    protected function _registerServices()
    {
        ...
        foreach ($appConfig->database as $id => $database) {
            switch (strtolower($database->adapter)) {
                case 'mysql':
                    $di->set($id, function() use ($database) {
                        return new \Phalcon\Db\Adapter\Pdo\Mysql($database->toArray());
                    });
                    break;
                case 'pgsql':
                    $di->set($id, function($database) {
        ...
        switch (strtolower($appConfig->session->adapter)) {
            case 'file':
                $di->setShared($id, function() {
                    return new \Phalcon\Session\Adapter\Files();
                });
                break;
            case 'memcached':
                $di->setShared($id, function() use ($di, $appConfig) {
